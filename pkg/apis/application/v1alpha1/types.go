...
type OperationState struct {
	...
	Phase common2.OperationPhase `json:"phase" protobuf:"bytes,2,opt,name=phase"`
...
type ResourceResult struct {
	...
	Status common2.ResultCode `json:"status,omitempty" protobuf:"bytes,6,opt,name=status"`
	// message for the last sync OR operation
	Message string `json:"message,omitempty" protobuf:"bytes,7,opt,name=message"`
	// the type of the hook, empty for non-hook resources
	HookType common2.HookType `json:"hookType,omitempty" protobuf:"bytes,8,opt,name=hookType"`
	// the state of any operation associated with this resource OR hook
	// note: can contain values for non-hook resources
	HookPhase common2.OperationPhase `json:"hookPhase,omitempty" protobuf:"bytes,9,opt,name=hookPhase"`
	// indicates the particular phase of the sync that this is for
	SyncPhase common2.SyncPhase `json:"syncPhase,omitempty" protobuf:"bytes,10,opt,name=syncPhase"`
}
...
func (r ResourceResults) Find(group string, kind string, namespace string, name string, phase common2.SyncPhase) (int, *ResourceResult) {
	for i, res := range r {
...
func (r ResourceResults) PruningRequired() (num int) {
	for _, res := range r {
		if res.Status == common2.ResultCodePruneSkipped {
			num++
		}
	}
	return num
